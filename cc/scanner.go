...
func (s *scanner) cppScan0() (tok Token) {
	...
	switch {
	case c > 0x7f && !unicode.IsSpace(c):
		s.eh("%v: non-ASCII character '%c' (U+%04X) found outside of comment or string literal", s.pos(s.off), c, c)
		return s.fail()
...
func (s *scanner) identifier() Token {
	...
	for {
		switch c := s.rune(); {
		case c > 0x7f:
			s.eh("%v: non-ASCII character '%c' (U+%04X) inside identifier", s.pos(s.off), c, c)
			return s.fail()
		case unicode.IsLetter(c) || c == '_' || unicode.IsDigit(c) || c == '$':
			s.shift()
		case c == '@':
			if s.peek(1) >= '0' && s.peek(1) <= '9' {
				s.shift() // consume '@'
