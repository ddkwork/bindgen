...
package cc // import "github.com/ddkwork/cc"
...
func newCtx(ast *AST, cfg *Config) *ctx {
	...
	c.builtinTypes = map[string]Type{
		...
		ts2String([]TypeSpecifierCase{TypeSpecifierInt8}):                                                             c.newPredefinedType(SChar),
		ts2String([]TypeSpecifierCase{TypeSpecifierInt8, TypeSpecifierSigned}):                                        c.newPredefinedType(SChar),
		ts2String([]TypeSpecifierCase{TypeSpecifierInt8, TypeSpecifierUnsigned}):                                      c.newPredefinedType(UChar),
		ts2String([]TypeSpecifierCase{TypeSpecifierInt16}):                                                            short,
		ts2String([]TypeSpecifierCase{TypeSpecifierInt16, TypeSpecifierSigned}):                                       short,
		ts2String([]TypeSpecifierCase{TypeSpecifierInt16, TypeSpecifierUnsigned}):                                     ushort,
		ts2String([]TypeSpecifierCase{TypeSpecifierInt32}):                                                            int,
		ts2String([]TypeSpecifierCase{TypeSpecifierInt32, TypeSpecifierSigned}):                                       int,
		ts2String([]TypeSpecifierCase{TypeSpecifierInt32, TypeSpecifierUnsigned}):                                     uint,
		ts2String([]TypeSpecifierCase{TypeSpecifierInt64}):                                                            longlong,
		ts2String([]TypeSpecifierCase{TypeSpecifierInt64, TypeSpecifierSigned}):                                       longlong,
		ts2String([]TypeSpecifierCase{TypeSpecifierInt64, TypeSpecifierUnsigned}):                                     ulonglog,
...
func (n *AST) check(cfg *Config) error {
	...
	var curEd *ExternalDeclaration
	defer func() {
		if err := recover(); err != nil {
			var posStr, srcStr string
			if curEd != nil {
				func() {
					defer func() { recover() }()
					posStr = curEd.Position().String()
				}()
				func() {
					defer func() { recover() }()
					srcStr = NodeSource(curEd)
				}()
				if srcStr == "" {
					func() {
						defer func() { recover() }()
						srcStr = curEd.Token.SrcStr()
					}()
				}
			}
			msg := fmt.Sprintf("cc check panic: %v", err)
			if posStr != "" {
				msg += fmt.Sprintf("\n  --> %s", posStr)
			}
			if srcStr != "" {
	...
	loop:
		for l := n.TranslationUnit; l != nil; l = l.TranslationUnit {
			...
			case ExternalDeclarationDecl:
				if ed.Declaration == nil || ed.Declaration.InitDeclaratorList == nil || ed.Declaration.InitDeclaratorList.InitDeclarator == nil || ed.Declaration.InitDeclaratorList.InitDeclarator.Declarator == nil {
					continue
				}
