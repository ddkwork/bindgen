...
func ClangMatchModuleDirectory(astNodeFilename, curFilename string) bool {
	if filepath.Clean(astNodeFilename) == filepath.Clean(curFilename) {
		return true
	}
	dir := filepath.Dir(curFilename)
	normalizedAst := filepath.Clean(astNodeFilename)
	prefix := filepath.Clean(dir) + string(filepath.Separator)
	return strings.HasPrefix(normalizedAst, prefix)
}
...
func parseHeader(node *AstNode, addNamePrefix string, output *CppParsedHeader, matcher HeaderMatcher) {
	...
	case "FunctionDecl":
		...
		fn, err := processFreeFunction(node, addNamePrefix)
		if err != nil {
			if errors.Is(err, ErrNoContent) {
				return
			}
		...
		output.FreeFunctions = append(output.FreeFunctions, fn)
	case "PragmaCommentDecl", "PragmaDetectMismatchDecl", "PragmaPackDecl", "PragmaMarkDecl":
		break
	default:
...
func processFreeFunction(node *AstNode, addNamePrefix string) (CppFreeFunction, error) {
	nodename, ok := node.Fields["name"].(string)
	if !ok {
		return CppFreeFunction{}, ErrNoContent
	}

	if strings.HasPrefix(nodename, "operator") {
		return CppFreeFunction{}, ErrNoContent
	}

	if strings.HasPrefix(nodename, "_") {
		return CppFreeFunction{}, ErrNoContent
	}

	var ret CppFreeFunction
	ret.FunctionName = addNamePrefix + nodename

	if typ, ok := node.Fields["type"].(map[string]interface{}); ok {
		qualType := getQualType(typ)
		if qualType != "" {
			var err error
			ret.ReturnType, ret.Parameters, _, _, err = ParseTypeString(qualType)
			if err != nil {
				return CppFreeFunction{}, err
			}
		}
	}
	...
	for _, inner := range node.Inner {
		if inner.Kind == "ParmVarDecl" {
			parmName, _ := inner.Fields["name"].(string)
			if parmName == "" {
				parmName = fmt.Sprintf("param%d", paramCounter+1)
			}
			if paramCounter < len(ret.Parameters) {
				ret.Parameters[paramCounter].ParameterName = parmName
			}
			paramCounter++
		}
	}

	if err := ActiveFilter.AllowFreeFunction(ret); err != nil {
		return CppFreeFunction{}, err
	}
	...
	if bases, ok := node.Fields["bases"].([]interface{}); ok {
		for _, base := range bases {
			...
			if typ, ok := base["type"].(map[string]interface{}); ok {
				qualType := getQualType(typ)
				if qualType != "" && ActiveFilter.AllowClass(qualType) {
...
nextMethod:
	for _, node := range node.Inner {
		...
		case "CXXConstructorDecl":
			...
			if !ActiveFilter.AllowCtor(ret.ClassName, mm) {
				continue
			}
		...
		case "CXXMethodDecl",
			...
			if i, ok := isPrivateSignal(&mm); ok {
				...
				// Check if this private signal should be allowed
				if err := ActiveFilter.AllowMethod(ret.ClassName, mm); err != nil {
					if errors.Is(err, ErrTooComplex) {
						continue nextMethod
					}
					// Real error
			...
			mm.IsSignal = isSignal && !mm.IsStatic && ActiveFilter.AllowSignal(mm)
		...
		case "FieldDecl":
			...
			if typobj, ok := node.Fields["type"].(map[string]interface{}); ok {
				...
				if qualType != "" {
					...
					if strings.Contains(fieldType.ParameterType, "unnamed") || strings.Contains(fieldType.ParameterType, "struct ") ||
						...
						strings.Contains(qualType, "(*)") {
						continue // Skip broken types and function pointers
					}
			...
			ret.Props = append(ret.Props, CppProperty{
				...
				PropertyType: fieldType.ParameterType,
				Visibility:   "public",
			})
...
nextEnumEntry:
	for _, entry := range node.Inner {
		kind := entry.Kind
		switch kind {
		case "DeprecatedAttr", "FullComment", "VisibilityAttr":
...
// getQualType extracts the best type string from a Clang type object.
// It prefers desugaredQualType over qualType because desugaredQualType
// contains fully qualified inner class names (e.g. ImVector<ImGuiTextFilter::ImGuiTextRange>
// instead of ImVector<ImGuiTextRange>).
func getQualType(typobj map[string]interface{}) string {
		...
		for _, methodObj := range methodInner {
			...
			case "ParmVarDecl":
				...
				if parmName == "" {
					...
					if len(mm.Parameters) == 1 && len(mm.MethodName) > 3 && strings.HasPrefix(mm.MethodName, "set") {
				...
				if paramCounter >= len(mm.Parameters) {
					break
				}
...
func ParseTypeString(typeString string) (CppParameter, []CppParameter, bool, string, error) {
	...
	// Block function pointer parameters - they contain (*)( or (*name)(
	if strings.Contains(typeString, "(*)") {
		return CppParameter{}, nil, false, "", ErrTooComplex
	}
