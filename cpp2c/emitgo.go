...
func (p CppParameter) RenderTypeGo(gfs *goFileState) string {
	...
	default:
		...
		if ft, ok := p.QFlagsOf(); ok {
			...
			if enumInfo, ok := KnownEnums.Get(ft.ParameterType); ok && enumInfo.PackageName != gfs.currentPackageName {
				// Cross-package
		...
		} else if enumInfo, ok := KnownEnums.Get(p.ParameterType); ok {
	...
	if pkg, ok := KnownClassnames.Get(p.ParameterType); ok && pkg.PackageName != gfs.currentPackageName {
...
func (gfs *goFileState) emitParameterGo2CABIForwarding(p CppParameter) (preamble string, rvalue string) {
	...
	if p.ParameterType == "QString" {
		...
		if classInfo, ok := KnownClassnames.Get(p.ParameterType); ok && gfs.currentPackageName != classInfo.PackageName {
			// Cross-package
...
func (gfs *goFileState) emitCabiToGo(assignExpr string, rt CppParameter, rvalue string) string {
	...
	if rt.Void() {
		...
		crossPackage := ""
		pkg, ok := KnownClassnames.Get(rt.ParameterType)
		if !ok {
...
func emitGo(src *CppParsedHeader, headerName string, packageName string) (string, string, error) {
	...
	platformRestriction := ActiveFilter.HeaderPlatformRestriction(headerName)
...
`)
	...
	nextEnum:
		for _, e := range src.Enums {
			...
			for _, ee := range e.Entries {
				...
				if KnownClassnames.Has(shortEnumName + "::" + ee.EntryName) {
					preventShortNames[e.EnumName] = struct{}{}
					continue nextEnum
				}
				if KnownEnums.Has(shortEnumName + "::" + ee.EntryName) {
					preventShortNames[e.EnumName] = struct{}{}
					continue nextEnum
				}
	...
	for _, c := range src.Classes {
		...
		for _, base := range c.DirectInherits {
			...
			} else if pkg, ok := KnownClassnames.Get(base); ok && pkg.PackageName != gfs.currentPackageName {
