...
BOOLEAN CallstackReturnAddressToCallingAddress(
    UCHAR *ReturnAddress, PUINT32 IndexOfCallFromReturnAddress) {
    ...
    if (ReturnAddress[-7] == 0xFF &&
      ...
      *IndexOfCallFromReturnAddress = 7;
      return TRUE;
               ...
               ((ReturnAddress[-5] & RmMask) == 0x90 ||
                (ReturnAddress[-5] & RmMask) == 0x98) &&
      ...
      *IndexOfCallFromReturnAddress = 4;
      return TRUE;
               ...
               ((ReturnAddress[-2] & RmMask) == 0x50 ||
                (ReturnAddress[-2] & RmMask) == 0x58) &&
      ...
      *IndexOfCallFromReturnAddress = 3;
      return TRUE;
               ...
               ((ReturnAddress[-1] & RmMask) == 0xD0 ||
                (ReturnAddress[-1] & RmMask) == 0xD8)) {
      *IndexOfCallFromReturnAddress = 2;
      return TRUE;
    } else if (ReturnAddress[-2] == 0xFF &&
               ((ReturnAddress[-1] & RmMask) == 0x10 ||
                (ReturnAddress[-1] & RmMask) == 0x18) &&
...
VOID CallstackShowFrames(PDEBUGGER_SINGLE_CALLSTACK_FRAME CallstackFrames,
  ...
  for (size_t i = 0; i < FrameCount; i++) {
    ...
    if (CallstackFrames[i].IsValidAddress) {
      if (CallstackFrames[i].IsExecutable &&
          CallstackReturnAddressToCallingAddress(
              (unsigned char *)&CallstackFrames[i]
                  .InstructionBytesOnRip[MAXIMUM_CALL_INSTR_SIZE],
              &CallLength)) {
      ...
      ShowMessages("[$+%03x] ",
                   i * (Is32Bit ? sizeof(UINT32) : sizeof(UINT64)));
      if (IsCall) {
        if (Is32Bit) {
