...
VOID CommandDumpHelp() {
  ...
  ShowMessages(
      "syntax : \t.dump [FromAddress (hex)] [ToAddress (hex)] [pid ProcessId "
      "(hex)] [path Path (string)]\n");
  ShowMessages(
  ...
  ShowMessages(
      "\t\te.g : .dump 401000 40b000 pid 1c0 path "
      "c:\\rev\\desktop\\dump2.dmp\n");
  ShowMessages(
      "\t\te.g : .dump fffff801deadb000 fffff801deade054 path "
      "c:\\rev\\dump3.dmp\n");
  ShowMessages(
      "\t\te.g : .dump fffff801deadb000 fffff801deade054 path "
      "c:\\rev\\dump4.dmp\n");
  ShowMessages(
      "\t\te.g : .dump 00007ff8349f2000 00007ff8349f8000 path "
      "c:\\rev\\dump5.dmp\n");
  ShowMessages(
      "\t\te.g : .dump @rax+@rcx @rax+@rcx+1000 path c:\\rev\\dump6.dmp\n");
...
VOID CommandDump(vector<CommandToken> CommandTokens, string Command) {
  ...
  if (StartAddress >= EndAddress) {
    ShowMessages(
        "err, please note that the 'to' address should be greater than the "
        "'from' address\n");
    return;
  }
  ...
  DumpFileHandle = CreateFileW(Filepath.c_str(), GENERIC_WRITE, 0, NULL,
  ...
  for (size_t i = 0; i <= Iterator; i++) {
    ...
    Length -= ActualLength;
    if (ActualLength != 0) {
      HyperDbgShowMemoryOrDisassemble(DEBUGGER_SHOW_COMMAND_DUMP, Address,
                                      MemoryType, READ_FROM_KERNEL, Pid,
                                      ActualLength, NULL);
    }
  }
...
VOID CommandDumpSaveIntoFile(PVOID Buffer, UINT32 Length) {
  ...
  if (DumpFileHandle == NULL) {
    ShowMessages(
        "err, invalid handle for saving the dump buffer is specified\n");
    return;
  }
