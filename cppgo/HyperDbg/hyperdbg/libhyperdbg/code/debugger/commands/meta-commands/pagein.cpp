...
VOID CommandPageinHelp() {
  ShowMessages(
      ".pagein : brings the page in, making it available in the RAM.\n\n");
  ShowMessages("syntax : \t.pagein [Mode (string)] [l Length (hex)]\n");
  ShowMessages(
      "syntax : \t.pagein [Mode (string)] [VirtualAddress (hex)] [l Length "
      "(hex)]\n");
  ShowMessages("\n");
...
BOOLEAN CommandPageinCheckAndInterpretModeString(
    const std::string &ModeString, PAGE_FAULT_EXCEPTION *PageFaultErrorCode) {
  std::unordered_set<char> AllowedChars = {'p', 'w', 'u', 'f',
                                           'k', 's', 'h', 'g'};
...
VOID CommandPageinRequest(UINT64 TargetVirtualAddrFrom,
                          ...
                          PAGE_FAULT_EXCEPTION PageFaultErrorCode, UINT32 Pid) {
  BOOL Status;
  ...
  if (g_IsSerialConnectedToRemoteDebuggee) {
    ...
    ShowMessages(
        "the '.pagein' command can be used ONLY in the debugger mode, "
    ...
    Status = DeviceIoControl(g_DeviceHandle, IOCTL_DEBUGGER_BRING_PAGES_IN,
                             &PageFaultRequest, SIZEOF_DEBUGGER_PAGE_IN_REQUEST,
                             &PageFaultRequest, SIZEOF_DEBUGGER_PAGE_IN_REQUEST,
                             &ReturnedLength, NULL);
    if (!Status) {
...
VOID CommandPagein(vector<CommandToken> CommandTokens, string Command) {
  ...
  for (auto Section : CommandTokens) {
    ...
    if (IsNextLength == TRUE) {
      if (!SymbolConvertNameOrExprToAddress(
              GetCaseSensitiveStringFromCommandToken(Section), &Length)) {
    ...
    if (CommandPageinCheckAndInterpretModeString(
            GetLowerStringFromCommandToken(Section), &PageFaultErrorCode)) {
      continue;
  ...
  CommandPageinRequest(TargetAddressFrom, TargetAddressTo, PageFaultErrorCode,
                       Pid);
}
