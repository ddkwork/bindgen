...
BOOLEAN HyperDbgReadAllRegisters(GUEST_REGS *GuestRegisters,
  ...
  SizeOfRegState = sizeof(DEBUGGEE_REGISTER_READ_DESCRIPTION) +
  ...
  if (RegState->KernelStatus == DEBUGGER_OPERATION_WAS_SUCCESSFUL) {
    Regs = (GUEST_REGS *)(((CHAR *)RegState) +
                          sizeof(DEBUGGEE_REGISTER_READ_DESCRIPTION));
    ExtraRegs =
        (GUEST_EXTRA_REGISTERS *)(((CHAR *)RegState) +
                                  sizeof(DEBUGGEE_REGISTER_READ_DESCRIPTION) +
                                  sizeof(GUEST_REGS));
...
BOOLEAN HyperDbgReadTargetRegister(REGS_ENUM RegisterId,
  ...
  if (!KdSendReadRegisterPacketToDebuggee(
          &RegState, sizeof(DEBUGGEE_REGISTER_READ_DESCRIPTION))) {
    return FALSE;
  }
  if (RegState.KernelStatus == DEBUGGER_OPERATION_WAS_SUCCESSFUL) {
...
BOOLEAN HyperDbgRegisterShowAll() {
  ...
  ShowMessages(
      ...
      "RFLAGS=%016llx\n",
      Regs.rax, Regs.rbx, Regs.rcx, Regs.rdx, Regs.rsi, Regs.rdi, ExtraRegs.RIP,
      ...
      Rflags.SignFlag ? "SF  1" : "SF  0", Rflags.ZeroFlag ? "ZF 1" : "ZF 0",
      Rflags.ParityFlag ? "PF 1" : "PF 0", Rflags.CarryFlag ? "CF 1" : "CF 0",
      Rflags.AuxiliaryCarryFlag ? "AXF 1" : "AXF 0", ExtraRegs.CS, ExtraRegs.SS,
      ExtraRegs.DS, ExtraRegs.ES, ExtraRegs.FS, ExtraRegs.GS, ExtraRegs.RFLAGS);
  return TRUE;
}
...
BOOLEAN HyperDbgRegisterShowTargetRegister(REGS_ENUM RegisterId) {
  ...
  ShowMessages("%s=%016llx\n", RegistersNames[RegisterId], TargetRegister);
  return TRUE;
}
...
VOID CommandR(vector<CommandToken> CommandTokens, string Command) {
  ...
  if (CommandTokens.size() == 1) {
    if (g_IsSerialConnectedToRemoteDebuggee) {
      ...
      ShowMessages(
          "err, reading registers (r) is not valid in the current "
