...
typedef BOOL(WINAPI *glpie_t)(LOGICAL_PROCESSOR_RELATIONSHIP,
                              PSYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX, PDWORD);

static SIZE_T GetWindowsCompatibleNumberOfCores() {
  SYSTEM_INFO SysInfo;
  GetSystemInfo(&SysInfo);
  return (SIZE_T)SysInfo.dwNumberOfProcessors;
}

static SIZE_T GetWindowsNumaNumberOfCores() {
  uint8_t *Buffer = NULL;
  SIZE_T NumCores = 0;
  DWORD Length = 0;
  HMODULE Kernel32 = GetModuleHandleW(L"kernel32.dll");
  glpie_t GetLogicalProcessorInformationEx =
      (glpie_t)GetProcAddress(Kernel32, "GetLogicalProcessorInformationEx");
  ...
  for (DWORD Offset = 0; Offset < Length;) {
    ...
    for (WORD group = 0; group < Info->Processor.GroupCount; ++group) {
      for (KAFFINITY Mask = Info->Processor.GroupMask[group].Mask; Mask != 0;
           Mask >>= 1) {
        NumCores += Mask & 1;
      }
    }
...
VOID CommandRdmsr(vector<CommandToken> CommandTokens, string Command) {
  ...
  Status = DeviceIoControl(
      g_DeviceHandle, IOCTL_DEBUGGER_READ_OR_WRITE_MSR, &MsrReadRequest,
      SIZEOF_DEBUGGER_READ_AND_WRITE_ON_MSR, OutputBuffer,
      (DWORD)(sizeof(UINT64) * NumCPU), &ReturnedLength, NULL);
  if (!Status) {
    ShowMessages(
