...
VOID CommandEventsHelp() {
  ...
  ShowMessages(
      "note : If you specify 'all' then e, d, or c will be applied to "
      "all of the events.\n\n");
  ShowMessages("\n");
...
VOID CommandEvents(vector<CommandToken> CommandTokens, string Command) {
  ...
  if (CompareLowerCaseStrings(CommandTokens.at(1), "e")) {
    ...
    if (CompareLowerCaseStrings(CommandTokens.at(2), "on")) {
      ...
      ShowMessages(
          "please specify a correct 'on' or 'off' state for the "
...
VOID CommandEventsShowEvents() {
  ...
  while (&g_EventTrace != TempList->Blink) {
    ...
    PDEBUGGER_GENERAL_EVENT_DETAIL CommandDetail = CONTAINING_RECORD(
    ...
    ShowMessages("%x\t(%s)\t    %s\n",
                 ...
                 CommandEventQueryEventState(CommandDetail->Tag) ? "enabled"
                                                                 : "disabled",
...
BOOLEAN CommandEventClearEvent(UINT64 Tag) {
  ...
  if (Result && Tag == DEBUGGER_MODIFY_EVENTS_APPLY_TO_ALL_TAG) {
    while ((UINT64)TmpCommandDetail != (UINT64)&g_EventTrace.Flink) {
      ...
      TmpCommandDetail = (PDEBUGGER_GENERAL_EVENT_DETAIL)
                             TmpCommandDetail->CommandsEventList.Flink;
      free(Temp);
    }
  ...
  if (ModifyEventRequest->KernelStatus == DEBUGGER_OPERATION_WAS_SUCCESSFUL) {
    if (ModifyEventRequest->TypeOfAction == DEBUGGER_MODIFY_EVENTS_ENABLE) {
      if (!CommandEventEnableEvent(Tag)) {
        ShowMessages(
            "err, the event was successfully, "
      ...
      if (!CommandEventClearEvent(Tag)) {
        ...
        if (g_IsSerialConnectedToRemoteDebuggee) {
          ShowMessages(
              "%s successfully cleared, but please note in the Debugger Mode "
              "(current mode), HyperDbg "
              "cannot clear events instantly. Instead, it first disables the "
              "events, and when "
              "you continue the debuggee (e.g., by pressing the 'g' command), "
              "the event will be cleared. "
              "Reapplying the same event without first continuing the debuggee "
              "may result in undefined behavior\n",
              Tag == DEBUGGER_MODIFY_EVENTS_APPLY_TO_ALL_TAG ? "events are"
                                                             : "the event is");
        }
        if (g_BreakPrintingOutput) {
          if (!g_IsConnectedToRemoteDebuggee) {
...
BOOLEAN CommandEventsModifyAndQueryEvents(
    UINT64 Tag, DEBUGGER_MODIFY_EVENTS_TYPE TypeOfAction) {
  BOOLEAN Status;
  ...
  if (g_IsSerialConnectedToRemoteDebuggee) {
    ...
    Status = DeviceIoControl(g_DeviceHandle, IOCTL_DEBUGGER_MODIFY_EVENTS,
                             &ModifyEventRequest, SIZEOF_DEBUGGER_MODIFY_EVENTS,
                             &ModifyEventRequest, SIZEOF_DEBUGGER_MODIFY_EVENTS,
                             &ReturnedLength, NULL);
    if (!Status) {
