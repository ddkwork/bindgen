...
BOOLEAN SymbolLocalReload(UINT32 UserProcessId) {
  ShowMessages("interpreting symbols and creating symbol maps\n");
  SymbolBuildSymbolTable(&g_SymbolTable, &g_SymbolTableSize, UserProcessId,
                         FALSE);
...
VOID SymbolCreateDisassemblerMapCallback(UINT64 Address, char *ModuleName,
                                         char *ObjectName,
  ...
  if (!g_DisassemblerSymbolMap.empty()) {
    ...
    if (Low == g_DisassemblerSymbolMap.end()) {
      ...
      if (Prev->second.ObjectSize >= Diff) {
        ...
        if (*UsedBaseAddress != Prev->first) {
          ShowMessages("%s+0x%x+0x%x", Prev->second.ObjectName.c_str(), Diff,
                       Diff - Prev->second.ObjectSize);
          *UsedBaseAddress = Prev->first;
          return TRUE;
        }
        return FALSE;
...
VOID SymbolBuildAndShowSymbolTable() {
  if (g_SymbolTable == NULL || g_SymbolTableSize == NULL) {
    ShowMessages(
        "err, symbol table is empty. please use '.sym reload' "
        "to build the symbol table\n");
    return;
  }
  for (size_t i = 0; i < g_SymbolTableSize / sizeof(MODULE_SYMBOL_DETAIL);
       i++) {
...
BOOLEAN SymbolLoadOrDownloadSymbols(BOOLEAN IsDownload, BOOLEAN SilentLoad) {
  ...
  if (!CommandSettingsGetValueFromConfigFile("SymbolServer", SymbolServer)) {
    ShowMessages(
        "please configure the symbol path (use '.help .sympath' for more "
        "information)\n");
    return FALSE;
  }
  if (g_SymbolTable == NULL || g_SymbolTableSize == NULL) {
    ShowMessages(
        "symbol table is empty, please use '.sym reload' to build a symbol "
        "table\n");
    return FALSE;
  }
  g_IsExecutingSymbolLoadingRoutines = TRUE;
  Result = ScriptEngineSymbolInitLoadWrapper(g_SymbolTable, g_SymbolTableSize,
                                             IsDownload, SymbolServer.c_str(),
                                             SilentLoad);
...
BOOLEAN SymbolConvertNameOrExprToAddress(const string &TextToConvert,
  ...
  if (!ConvertStringToUInt64(TextToConvert, &Address)) {
    ...
    Address = ScriptEngineConvertNameToAddressWrapper(
        ConstTextToConvert.c_str(), &IsFound);
    if (!IsFound) {
...
BOOLEAN SymbolBuildSymbolTable(PMODULE_SYMBOL_DETAIL *BufferToStoreDetails,
                               PUINT32 StoredLength, UINT32 UserProcessId,
  ...
  transform(SystemRootString.begin(), SystemRootString.end(),
  ...
  ModuleSymDetailArray = (PMODULE_SYMBOL_DETAIL)malloc(
      (ModuleInfo->NumberOfModules + ModulesCount) *
  ...
  RtlZeroMemory(ModuleSymDetailArray,
                (ModuleInfo->NumberOfModules + ModulesCount) *
                    sizeof(MODULE_SYMBOL_DETAIL));
  if (ModulesCount != 0) {
    for (UINT32 i = 0; i < ModulesCount; i++) {
      ...
      if (ScriptEngineConvertFileToPdbFileAndGuidAndAgeDetailsWrapper(
              TempPath, ModuleSymbolPath, ModuleSymbolGuidAndAge,
      ...
      if (IsSymbolPdbDetailAvailable) {
        ...
        memcpy(ModuleSymDetailArray[i].ModuleSymbolGuidAndAge,
               ModuleSymbolGuidAndAge, MAXIMUM_GUID_AND_AGE_SIZE);
        memcpy(ModuleSymDetailArray[i].ModuleSymbolPath, ModuleSymbolPath,
               MAX_PATH);
      ...
      if (SendOverSerial) {
        KdSendSymbolDetailPacket(&ModuleSymDetailArray[i], i,
                                 ModuleInfo->NumberOfModules + ModulesCount);
      }
    }
  }
  for (UINT32 i = 0; i < ModuleInfo->NumberOfModules; i++) {
    ...
    auto PathName = ModuleInfo->Modules[i].FullPathName +
    ...
    if (ScriptEngineConvertFileToPdbFileAndGuidAndAgeDetailsWrapper(
            ModuleFullPath.c_str(), ModuleSymbolPath, ModuleSymbolGuidAndAge,
            FALSE)) {
      IsSymbolPdbDetailAvailable = TRUE;
    } else {
      IsSymbolPdbDetailAvailable = FALSE;
    }
    ModuleSymDetailArray[IndexInSymbolBuffer].BaseAddress =
        (UINT64)ModuleInfo->Modules[i].ImageBase;
    memcpy(ModuleSymDetailArray[IndexInSymbolBuffer].FilePath,
           ModuleFullPath.c_str(), ModuleFullPath.size());
    if (IsSymbolPdbDetailAvailable) {
      ModuleSymDetailArray[IndexInSymbolBuffer].IsSymbolDetailsFound = TRUE;
      memcpy(ModuleSymDetailArray[IndexInSymbolBuffer].ModuleSymbolGuidAndAge,
             ModuleSymbolGuidAndAge, MAXIMUM_GUID_AND_AGE_SIZE);
      memcpy(ModuleSymDetailArray[IndexInSymbolBuffer].ModuleSymbolPath,
             ModuleSymbolPath, MAX_PATH);
      string ModuleSymbolPathString(ModuleSymbolPath);
      if (ModuleSymbolPathString.find(":\\") != std::string::npos)
    ...
    if (SendOverSerial) {
      KdSendSymbolDetailPacket(&ModuleSymDetailArray[IndexInSymbolBuffer], i,
                               ModuleInfo->NumberOfModules + ModulesCount);
    }
  }
  *BufferToStoreDetails = ModuleSymDetailArray;
  *StoredLength = (ModuleInfo->NumberOfModules + ModulesCount) *
                  sizeof(MODULE_SYMBOL_DETAIL);
  free(ModuleInfo);
  if (IsFreeUsermodeModulesBuffer) {
    free(ModuleDetailsRequest);
...
BOOLEAN SymbolBuildAndUpdateSymbolTable(PMODULE_SYMBOL_DETAIL SymbolDetail) {
  if (g_SymbolTableCurrentIndex >= MAXIMUM_SUPPORTED_SYMBOLS) {
    ShowMessages(
        "err, the symbol table buffer is full, unable to add new symbol\n");
    return FALSE;
  }
  if (g_SymbolTable == NULL) {
    g_SymbolTable = (PMODULE_SYMBOL_DETAIL)malloc(MAXIMUM_SUPPORTED_SYMBOLS *
                                                  sizeof(MODULE_SYMBOL_DETAIL));
    if (g_SymbolTable == NULL) {
      ShowMessages("err, unable to allocate memory for module list (%x)\n",
                   GetLastError());
    ...
    g_SymbolTableCurrentIndex = 0;
    RtlZeroMemory(g_SymbolTable,
                  MAXIMUM_SUPPORTED_SYMBOLS * sizeof(MODULE_SYMBOL_DETAIL));
  }
  memcpy(&g_SymbolTable[g_SymbolTableCurrentIndex], SymbolDetail,
         sizeof(MODULE_SYMBOL_DETAIL));
...
BOOLEAN SymbolCheckAndAllocateModuleInformation(PRTL_PROCESS_MODULES *Modules) {
  ...
  Status = NtQuerySystemInformation(
      (SYSTEM_INFORMATION_CLASS)SystemModuleInformation, NULL, NULL,
  ...
  *Modules = (PRTL_PROCESS_MODULES)malloc(SysModuleInfoBufferSize);
  ...
  Status = NtQuerySystemInformation(
      (SYSTEM_INFORMATION_CLASS)SystemModuleInformation, *Modules,
      SysModuleInfoBufferSize, NULL);
