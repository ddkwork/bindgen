...
VOID RemoteConnectionListen(PCSTR Port) {
  ...
  while (true) {
    ...
    RemoteConnectionSendResultsToHost((const char *)g_EndOfBufferCheckTcp,
                                      sizeof(g_EndOfBufferCheckTcp));
...
VOID RemoteConnectionConnect(PCSTR Ip, PCSTR Port) {
  ...
  if (CommunicationClientConnectToServer(Ip, Port, &g_ClientConnectSocket) ==
    ...
    if (CommunicationClientSendMessage(g_ClientConnectSocket,
                                       (const char *)BuildSignature,
                                       sizeof(BuildSignature)) != 0) {
      ShowMessages("err, failed to communicate with debuggee\n");
      return;
    }
    if (CommunicationClientReceiveMessage(g_ClientConnectSocket, Recv,
                                          sizeof(Recv), &BuffRecv) != 0) {
...
int RemoteConnectionSendCommand(const char *sendbuf, int len) {
  ...
  WaitForSingleObject(g_EndOfMessageReceivedEvent, INFINITE);
  return 0;
}
