...
VOID CommandPcicamHelp() {
  ShowMessages(
      "!pcicam : dumps the PCI configuration space (CAM) for a given "
      "device.\n\n");
  ShowMessages(
      "syntax : \t!pcicam [Bus (hex)] [Device (hex)] [Function (hex)]\n");
  ShowMessages(
      "syntax : \t!pcicam [Bus (hex)] [Device (hex)] [Function (hex)] [Dump "
      "(string)]\n");
  ShowMessages("\n");
...
VOID CommandPcicam(vector<CommandToken> CommandTokens, string Command) {
  ...
  const char *PciHeaderTypes[] = {"Endpoint", "PCI-to-PCI Bridge",
  ...
  if (g_IsSerialConnectedToRemoteDebuggee) {
    ...
    Status = DeviceIoControl(
        g_DeviceHandle, IOCTL_PCIDEVINFO_ENUM, &PcidevinfoPacket,
        SIZEOF_DEBUGGEE_PCIDEVINFO_REQUEST_RESPONSE_PACKET, &PcidevinfoPacket,
        SIZEOF_DEBUGGEE_PCIDEVINFO_REQUEST_RESPONSE_PACKET, &ReturnedLength,
        NULL);
    if (!Status) {
    ...
    if (PcidevinfoPacket.KernelStatus == DEBUGGER_OPERATION_WAS_SUCCESSFUL) {
      const char *PciHeaderTypeAsString[] = {"Endpoint", "PCI-to-PCI Bridge",
                                             "PCI-to-CardBus Bridge"};
      ...
      if (!PcidevinfoPacket.PrintRaw) {
        ...
        if ((PcidevinfoPacket.DeviceInfo.ConfigSpace.CommonHeader.HeaderType &
              ...
              if (((PcidevinfoPacket.DeviceInfo.ConfigSpace.DeviceHeader
                ...
                ShowMessages(
                    ...
                    ((PcidevinfoPacket.DeviceInfo.ConfigSpace.CommonHeader
                      ...
                      0x2) >>
                         1 ==
                     0) || !PcidevinfoPacket.DeviceInfo.MmioBarInfo[i].IsEnabled
                        ? "[disabled]"
                        : "",
          ...
          ShowMessages(
              ...
              "(0xD): %06x\nReserved (0xE): %08x\nInterrupt Line: "
              "%02x\nInterrupt Pin: %02x\nMin Grant: %02x\nMax latency: %02x\n",
              PcidevinfoPacket.DeviceInfo.ConfigSpace.DeviceHeader.ConfigSpaceEp
                  .CardBusCISPtr,
              PcidevinfoPacket.DeviceInfo.ConfigSpace.DeviceHeader.ConfigSpaceEp
                  .SubVendorId,
              PcidevinfoPacket.DeviceInfo.ConfigSpace.DeviceHeader.ConfigSpaceEp
