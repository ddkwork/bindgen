...
VOID ThreadHolderAllocateThreadHoldingBuffers() {
  PoolManagerRequestAllocation(sizeof(USERMODE_DEBUGGING_THREAD_HOLDER), 2,
                               PROCESS_THREAD_HOLDER);
  PoolManagerCheckAndPerformAllocationAndDeallocation();
}

BOOLEAN ThreadHolderAssignThreadHolderToProcessDebuggingDetails(
    PUSERMODE_DEBUGGING_PROCESS_DETAILS ProcessDebuggingDetail) {
  PUSERMODE_DEBUGGING_THREAD_HOLDER ThreadHolder;
  InitializeListHead(&ProcessDebuggingDetail->ThreadsListHead);
  ThreadHolder = (USERMODE_DEBUGGING_THREAD_HOLDER*)PoolManagerRequestPool(
      PROCESS_THREAD_HOLDER, TRUE, sizeof(USERMODE_DEBUGGING_THREAD_HOLDER));
  if (!ThreadHolder) {
    return FALSE;
  }
  InsertHeadList(&ProcessDebuggingDetail->ThreadsListHead,
                 &(ThreadHolder->ThreadHolderList));
  return TRUE;
}

BOOLEAN ThreadHolderIsAnyPausedThreadInProcess(
    PUSERMODE_DEBUGGING_PROCESS_DETAILS ProcessDebuggingDetail) {
  ...
  while (&ProcessDebuggingDetail->ThreadsListHead != TempList->Flink) {
    ...
    PUSERMODE_DEBUGGING_THREAD_HOLDER ThreadHolder = CONTAINING_RECORD(
        TempList, USERMODE_DEBUGGING_THREAD_HOLDER, ThreadHolderList);
...
ThreadHolderGetProcessThreadDetailsByProcessIdAndThreadId(UINT32 ProcessId,
  ...
  ProcessDebuggingDetail =
      AttachingFindProcessDebuggingDetailsByProcessId(ProcessId);
...
ThreadHolderFindOrCreateThreadDebuggingDetail(
  ...
  PUSERMODE_DEBUGGING_THREAD_HOLDER NewThreadHolder =
      (USERMODE_DEBUGGING_THREAD_HOLDER*)PoolManagerRequestPool(
          PROCESS_THREAD_HOLDER, TRUE,
          sizeof(USERMODE_DEBUGGING_THREAD_HOLDER));
  if (NewThreadHolder == NULL) {
    LogError(
        "Err, enable to find a place to save the threads data, "
  ...
  InsertHeadList(&ProcessDebuggingDetail->ThreadsListHead,
                 &(NewThreadHolder->ThreadHolderList));
  SpinlockUnlock(&VmxRootThreadHoldingLock);
  return &NewThreadHolder->Threads[0];
}

BOOLEAN ThreadHolderApplyActionToPausedThreads(
    PUSERMODE_DEBUGGING_PROCESS_DETAILS ProcessDebuggingDetails,
  ...
  if (!ActionRequest->ApplyToAllPausedThreads) {
    ThreadDebuggingDetails =
        ThreadHolderGetProcessThreadDetailsByProcessIdAndThreadId(
            ProcessDebuggingDetails->ProcessId, ActionRequest->TargetThreadId);
      ...
      for (size_t i = 0; i < MAX_THREADS_IN_A_PROCESS_HOLDER; i++) {
        if (ThreadHolder->Threads[i].ThreadId != NULL_ZERO &&
          ...
          for (size_t j = 0; j < MAX_USER_ACTIONS_FOR_THREADS; j++) {
            if (ThreadHolder->Threads[i].UdAction[j].ActionType ==
                DEBUGGER_UD_COMMAND_ACTION_TYPE_NONE) {
              ThreadHolder->Threads[i].UdAction[j].OptionalParam1 =
                  ActionRequest->UdAction.OptionalParam1;
              ThreadHolder->Threads[i].UdAction[j].OptionalParam2 =
                  ActionRequest->UdAction.OptionalParam2;
              ThreadHolder->Threads[i].UdAction[j].OptionalParam3 =
                  ActionRequest->UdAction.OptionalParam3;
              ThreadHolder->Threads[i].UdAction[j].OptionalParam4 =
                  ActionRequest->UdAction.OptionalParam4;
              ThreadHolder->Threads[i].UdAction[j].ActionType =
                  ActionRequest->UdAction.ActionType;
              CommandApplied = TRUE;
              break;
            }
...
VOID ThreadHolderQueryDetailsOfActiveDebuggingThreadsAndProcesses(
  ...
  while (&g_ProcessDebuggingDetailsListHead != TempList->Flink) {
    ...
    BufferToStoreDetails[CurrentIndex].ProcessId =
    ...
    while (&ProcessDebuggingDetails->ThreadsListHead != TempList2->Flink) {
      ...
      for (size_t i = 0; i < MAX_THREADS_IN_A_PROCESS_HOLDER; i++) {
        if (ThreadHolder->Threads[i].IsPaused) {
          ...
          BufferToStoreDetails[CurrentIndex].ProcessId =
              ProcessDebuggingDetails->ProcessId;
          BufferToStoreDetails[CurrentIndex].ThreadId =
              ThreadHolder->Threads[i].ThreadId;
          CurrentIndex++;
          if (MaxCount == CurrentIndex) {
            return;
          }
