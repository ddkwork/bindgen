...
IMPORT_EXPORT_HYPERDBG_SYMBOL_PARSER UINT32 SymLoadFileSymbol(
...
IMPORT_EXPORT_HYPERDBG_SYMBOL_PARSER BOOLEAN SymConvertFileToPdbPath(
...
IMPORT_EXPORT_HYPERDBG_SYMBOL_PARSER BOOLEAN SymbolInitLoad(
    PVOID BufferToStoreDetails, UINT32 StoredLength,
    BOOLEAN DownloadIfAvailable, const char *SymbolPath, BOOLEAN IsSilentLoad);
IMPORT_EXPORT_HYPERDBG_SYMBOL_PARSER BOOLEAN SymShowDataBasedOnSymbolTypes(
    const char *TypeName, UINT64 Address, BOOLEAN IsStruct, PVOID BufferAddress,
    const char *AdditionalParameters);
IMPORT_EXPORT_HYPERDBG_SYMBOL_PARSER BOOLEAN SymQuerySizeof(
    _In_ const char *StructNameOrTypeName, _Out_ UINT32 *SizeOfField);
IMPORT_EXPORT_HYPERDBG_SYMBOL_PARSER BOOLEAN SymCastingQueryForFiledsAndTypes(
    _In_ const char *StructName, _In_ const char *FiledOfStructName,
    _Out_ PBOOLEAN IsStructNamePointerOrNot,
    _Out_ PBOOLEAN IsFiledOfStructNamePointerOrNot,
    _Out_ char **NewStructOrTypeName, _Out_ UINT32 *OffsetOfFieldFromTop,
