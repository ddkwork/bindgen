...
#define LIST_FOR_EACH(_head, _struct_type, _var) \
  _LIST_FOR_EACH(_head, _struct_type, Link, _var)
#define LIST_FOR_NEXT(_start, _head, _type, _var) \
...
#define _LIST_FOR_EACH(_head, _type, _member, _var)                  \
  for (_type *_var = CONTAINING_RECORD(_head.Flink, _type, _member), \
             *UNIQUE_NAME(_n) = _NEXT_ENTRY(_var, _member, _type);   \
       &_var->_member != &_head; _var = UNIQUE_NAME(_n),             \
             UNIQUE_NAME(_n) = _NEXT_ENTRY(_var, _member, _type))
#define _LIST_FOR_NEXT(_start, _head, _type, _member, _var)        \
  for (_type *_var = _NEXT_ENTRY(_start, _member, _type),          \
             *UNIQUE_NAME(_n) = _NEXT_ENTRY(_var, _member, _type); \
       &_var->_member != &_head; _var = UNIQUE_NAME(_n),           \
             UNIQUE_NAME(_n) = _NEXT_ENTRY(_var, _member, _type))
