...
NTKERNELAPI
PVOID
NTAPI RtlPcToFileHeader(_In_ PVOID PcValue, _Out_ PVOID *BaseOfImage);
NTKERNELAPI
PIMAGE_NT_HEADERS
NTAPI RtlImageNtHeader(_In_ PVOID ImageBase);
...
VOID Print(_In_ PCCH Format, _In_...) {
  ...
  const int n = _vsnprintf_s(message, sizeof(message), sizeof(message) - 1,
                             Format, argList);
  message[n] = '\0';
  vDbgPrintExWithPrefix("[ZYDIS] ", DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
                        message, argList);
  va_end(argList);
}
...
_Use_decl_annotations_
    ...
    NTSTATUS
    DriverEntryTest(_In_ PDRIVER_OBJECT DriverObject,
  ...
  if (imageBase == 0) return STATUS_DRIVER_ENTRYPOINT_NOT_FOUND;
  const PIMAGE_NT_HEADERS ntHeaders = RtlImageNtHeader((PVOID)imageBase);
  if (ntHeaders == NULL) return STATUS_INVALID_IMAGE_FORMAT;
  ...
  if (initSection == NULL) return STATUS_NOT_FOUND;
  const ULONG entryPointRva =
      (ULONG)((ULONG_PTR)DriverObject->DriverInit - imageBase);
  const ULONG importDirRva =
      ntHeaders->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT]
          .VirtualAddress;
  SIZE_T length =
      initSection->VirtualAddress + initSection->SizeOfRawData - entryPointRva;
  if (importDirRva > entryPointRva &&
      importDirRva > initSection->VirtualAddress &&
  ...
  while ((status = ZydisDecoderDecodeFull(
              &decoder, (PVOID)(imageBase + entryPointRva + readOffset),
