...

{
  ...
  PL011_WRITE_REGISTER_USHORT((PUSHORT)(Port->Address + UART_CR), 0,
                              Force32Bit);
  do {
  ...
  } while ((Fsr & (UART_FR_RXFE | UART_FR_TXFE)) !=
           (UART_FR_RXFE | UART_FR_TXFE));
  PL011_WRITE_REGISTER_UCHAR(Port->Address + UART_LCRH,
                             (UART_LCRH_WLEN_8 | UART_LCRH_FEN), Force32Bit);
  PL011_WRITE_REGISTER_USHORT((PUSHORT)(Port->Address + UART_IMSC), 0x0000,
                              Force32Bit);
  PL011_WRITE_REGISTER_USHORT((PUSHORT)(Port->Address + UART_ICR), 0x07FF,
                              Force32Bit);
...

{
  ...
  if ((Fsr & UART_FR_RXFE) == 0) {
    Value = PL011_READ_REGISTER_USHORT((PUSHORT)(Port->Address + UART_DR),
                                       Force32Bit);
...

{
  ...
  Force32Bit = ((Port->Flags & PORT_FORCE_32BIT_IO) != 0);
  if (BusyWait != FALSE) {
    while (PL011_READ_REGISTER_USHORT((PUSHORT)(Port->Address + UART_FR),
                                      Force32Bit) &
           (UART_FR_TXFF));
  } else {
    if (PL011_READ_REGISTER_USHORT((PUSHORT)(Port->Address + UART_FR),
                                   Force32Bit) &
        (UART_FR_TXFF)) {
      return UartNotReady;
    }
...

{
  ...
  Force32Bit = ((Port->Flags & PORT_FORCE_32BIT_IO) != 0);
  Flags =
      PL011_READ_REGISTER_USHORT((PUSHORT)(BaseAddress + UART_FR), Force32Bit);
...
UART_HARDWARE_DRIVER PL011HardwareDriver = {PL011InitializePort, PL011SetBaud,
                                            PL011GetByte, PL011PutByte,
                                            PL011RxReady};
UART_HARDWARE_DRIVER SBSAHardwareDriver = {
    SBSAInitializePort, PL011SetBaud, PL011GetByte, PL011PutByte, PL011RxReady};
UART_HARDWARE_DRIVER SBSA32HardwareDriver = {SBSA32InitializePort, PL011SetBaud,
                                             PL011GetByte, PL011PutByte,
                                             PL011RxReady};
